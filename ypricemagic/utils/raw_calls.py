...
def _balanceOf(call_address, input_address, block=None):
    data = raw_call(call_address, "balanceOf(address)", block=block, input=input_address)
    return convert.to_int(data)
...
def raw_call(contract_address: str, method: str, block=None, inputs=None):
    '''
...
def prepare_input(input):
    try: return '000000000000000000000000' + convert.to_address(input)
    except: raise 
