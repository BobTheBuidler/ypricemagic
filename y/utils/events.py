...
def decode_logs(logs: Union[Iterable[LogReceipt], Iterable[Log]]) -> EventDict:
    ...
    try:
        decoded = _decode_logs([dict(log.items()) for log in logs] if special_treatment else logs)
    except Exception:
        decoded = []
        for log in logs:
            with reraise_excs_with_extra_context(log):
                # get some help for debugging
                decoded.extend(_decode_logs([dict(log.items()) if special_treatment else log]))
