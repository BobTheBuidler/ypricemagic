...
multicall = None
multicall2 = brownie.Contract.from_abi("Multicall2",MULTICALL2, MULTICALL2_ABI) if brownie.chain.id in [Network.Harmony,Network.Cronos] else Contract(MULTICALL2)
...
async def multicall_decimals(
    ...
    return await a_sync.map(_decimals, addresses, block=block, return_None_on_failure=return_None_on_failure).values(pop=True)
