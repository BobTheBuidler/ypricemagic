...
class GenericAmm:
    ...
    def get_price(self, lp_token_address: str, block: Optional[Block] = None) -> UsdPrice:
    ...
    def get_tokens(self, lp_token_address: str) -> Tuple[EthAddress,EthAddress]:
        lp_token_contract = Contract(lp_token_address)
        tokens = fetch_multicall(*[[lp_token_contract,attr] for attr in ['token0', 'token1']])
        return ERC20(tokens[0]), ERC20(tokens[1])
    
    def get_tvl(self, lp_token_address: str, block: Optional[Block] = None) -> UsdValue:
        lp_token_contract = Contract(lp_token_address)
