...
async def is_curve_gauge(token_address: AnyAddressType) -> bool:
    ...
    2. A call to ``lp_token()`` on the contract succeeds and returns a non-zero address.

    Both conditions must be satisfied.  The symbol check is a cheap fast-path;
    the ``lp_token()`` call confirms the contract actually implements the Curve
    gauge interface (all V1–V4, RewardsOnly, and NG gauges expose this method).
    ...
    # Confirm the contract exposes lp_token() — present on all Curve gauge versions
    lp = await get_lp_token(token_address, sync=False)
...
async def get_lp_token(token_address: AnyAddressType) -> ChecksumAddress | None:
    """Return the underlying Curve LP token address for a gauge.

    Calls ``lp_token()`` on the gauge contract.  Returns ``None`` if the call
    reverts or returns the zero address (i.e. the contract is not a gauge).

    Args:
        token_address: The gauge contract address.

    Returns:
        The LP token address, or ``None`` if not available.

    Examples:
        >>> await get_lp_token("0xcF5136C67fA8A375BaBbDf13c0307EF994b5681D", sync=False)
...
async def get_price(
    ...
    Args:
        ...
        block: The block number at which to get the price. Defaults to the latest block.
        skip_cache: Whether to skip the cache when fetching the price.
            Defaults to :obj:`ENVS.SKIP_CACHE`.

    Returns:
        The USD price of the gauge (equals the LP token price).

    Examples:
        Synchronous usage:
            >>> get_price("0xcF5136C67fA8A375BaBbDf13c0307EF994b5681D")

        Asynchronous usage:
            >>> await get_price("0xcF5136C67fA8A375BaBbDf13c0307EF994b5681D", sync=False)

    See Also:
        - :func:`get_lp_token` for retrieving the underlying LP token address.
        - :func:`y.prices.magic.get_price` for the underlying price fetching logic.
    """
    from y.prices import magic

    token_address = await convert.to_address_async(token_address)
    lp_token = await get_lp_token(token_address, sync=False)
    return await magic.get_price(lp_token, block=block, skip_cache=skip_cache, sync=False)
