...
Pricing strategy:
  - With ``amount``: call ``previewRedeem(actual_amount_in_shares)`` — captures real fees.
  - Without ``amount``: call ``previewRedeem(10**decimals)`` for per-share spot rate.
...
async def is_erc4626_vault(token_address: AnyAddressType) -> bool:
    ...
    return await has_methods(
        token_address,
        ("asset()(address)", "previewRedeem(uint256)(uint256)"),
        all,
        sync=False,
    )
...
async def get_price(
    ...
    ``assets_received / share_scale * underlying_price``

    Where ``assets_received`` comes from ``previewRedeem``, falling back to
    ``convertToAssets`` if ``previewRedeem`` reverts.  If both revert, ``None``
    is returned so the caller can fall through to other price sources.
    ...
    Args:
        ...
        amount: Human-readable amount of vault tokens being redeemed
            (e.g. ``1000`` for 1 000 tokens).  When given,
            ``previewRedeem(amount_in_shares)`` is called to capture real
            fee/slippage for that size.  When ``None``, one full share unit
            (``10**decimals``) is used for a spot per-share rate.
    ...
    Examples:
        Synchronous usage:
            >>> get_price("0x83F20F44975D03b1b09e64809B757c47f942BEeA")

        Asynchronous usage:
            >>> await get_price("0x83F20F44975D03b1b09e64809B757c47f942BEeA", sync=False)
    ...
    scale = await vault_erc20.__scale__

    if amount is not None:
        shares_to_redeem = int(Decimal(str(amount)) * Decimal(str(scale)))
    else:
        # Spot price: one full share
        shares_to_redeem = int(scale)
    ...
    #                         divided by (shares_to_redeem / vault_scale) to get per-unit price
    assets_per_share = (
        Decimal(assets_received)
        / Decimal(underlying_scale)
        / (Decimal(shares_to_redeem) / Decimal(scale))
    )
    return UsdPrice(assets_per_share * Decimal(float(underlying_price)))
