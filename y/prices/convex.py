...
2. **Dynamic detection** – for tokens not in the static mapping, the module
   ...
   ``poolInfo(i)`` entries until it finds the pool whose ``pool_token`` matches
   the queried address.  The corresponding ``pool_lptoken`` is returned as the
   underlying Curve LP.
...
async def is_convex_lp(token_address: AnyAddressType) -> bool:
    ...
    2. **Dynamic path** – for tokens not in the static mapping:

       a. Check that the token symbol starts with ``"cvx"`` (cheap, cached).
       b. Call ``operator()`` on the token to obtain the Convex booster address.
       c. Verify the booster exposes ``poolLength()`` (no-input view function),
          confirming it is a Convex booster contract.
       d. Iterate the booster's pool list via ``poolInfo(i)`` to confirm the token
          is registered as a Convex deposit token.
    ...
    # Dynamic path: symbol prefix check (cheap — uses cached ERC20 symbol)
    try:
        symbol = await ERC20(token_address, asynchronous=True).symbol
    except Exception:
        return False

    if not symbol or not symbol.lower().startswith("cvx"):
        return False

    # Confirm dynamic detection succeeds (operator → booster → pool list)
...
async def get_underlying_lp(token_address: AnyAddressType) -> ChecksumAddress | None:
    ...
    4. Iterates ``poolInfo(i)`` for each pool until it finds the one whose
       ``pool_token`` matches ``token_address``.
    5. Returns the ``pool_lptoken`` (the underlying Curve LP token).
    ...
    for i in range(num_pools):
        ...
        # pool_info[1] is the pool_token (Convex deposit token / cvxToken)
