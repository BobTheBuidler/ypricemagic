...
- **xTAROT**: Symbol == ``'xTAROT'``, has ``shareValuedAsUnderlying()`` and ``underlying()``.
  Price = underlying × shareValuedAsUnderlying(1e18) / 1e18.
...
async def is_pickle_pslp(token_address: AnyAddressType) -> bool:
    ...
    Examples:
        >>> is_pickle_pslp("0x55282dA27a3a02eFe599f9bD85E2e0C78f9cD2b2")
        True
    """
...
async def get_price_pool_together_v4(
    ...
    return await magic.get_price(underlying_address, block=block, skip_cache=skip_cache, sync=False)
...
async def get_price_tarot_vault(
    ...
    Price = underlying_price × shareValuedAsUnderlying(1e18) / 1e18.
    ...
    # underlying_raw represents how much underlying (in wei) you get per 1e18 shares
    # We need the underlying token's decimals to properly scale
    underlying_erc20 = ERC20(underlying_address, asynchronous=True)
    underlying_scale = await underlying_erc20.__scale__

    # price_per_share = (underlying_raw / underlying_scale) / (scale / share_scale)
    # Since we use 1e18 as our input share amount, and shares are 18 decimals:
    # price_per_share = (underlying_raw / underlying_scale)
    # (the 1e18 input shares cancel out the 18-decimal share denomination)
