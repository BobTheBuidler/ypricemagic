...
async def get_block_timestamp_async(height: int) -> int:
    ...
    if client in ['tg', 'erigon']:
        header = await dank_web3.manager.coro_request(f"{client}_getHeaderByNumber", [height])
        ts = int(header.timestamp, 16)
    else:
        block = await dank_web3.eth.get_block(height)
        ts = block.timestamp
...
async def check_node_async() -> None:
    if GANACHE_FORK:
        return
    current_time = time.time()
    latest = await dank_web3.eth.get_block('latest')
